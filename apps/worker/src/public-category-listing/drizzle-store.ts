...
import type {
  ...
  ListCategoryArticleCandidatesOptions,
  PublicCategoryListingExecutor,
  PublicCategoryListingStore,
  PublicCategoryListingTransaction,
} from "./types.js";

export function createDrizzlePublicCategoryListingStore(
  db: TopicpressDatabase,
): PublicCategoryListingStore {
  return {
    transaction: (callback) =>
      db.transaction((tx) => callback(createDrizzlePublicCategoryListingTransaction(tx))),
  };
}

function createDrizzlePublicCategoryListingTransaction(
  db: PublicCategoryListingExecutor,
): PublicCategoryListingTransaction {
  return {
    findActiveCategoryByConfigKey: (configKey) => findActiveCategoryByConfigKey(db, configKey),
    listCategoryArticleCandidates: (options) => listCategoryArticleCandidates(db, options),
  };
}
...
async function listCategoryArticleCandidates(
  db: PublicCategoryListingExecutor,
  ...
  const baseRows = await db
    ...
    .where(
      and(
        ...
        eq(articles.categoryId, options.categoryId),
