...
import { listHomepageArticles, type HomepageArticle } from "@topicpress/worker";
import { drizzle, type PostgresJsDatabase } from "drizzle-orm/postgres-js";
import postgres from "postgres";

const homepageArticleLimit = 12;
...
export async function getHomepageArticles(locale: string): Promise<readonly HomepageArticle[]> {
  const client = createDatabaseClient();

  try {
    return await listHomepageArticles(client.db, { locale, limit: homepageArticleLimit });
  } finally {
    await client.close();
  }
}
