...
const categoryListingArticleLimit = 12;
...
export async function getPublicCategoryListing(
  locale: string,
  categorySlug: string,
  ...
  try {
    return await getCategoryListing(client.db, {
      locale,
      categorySlug,
      limit: categoryListingArticleLimit,
    });
  } finally {
    await client.close();
  }
...
export type { CategoryListingResult, HomepageArticle };
