...
export async function getReviewableArticles(): Promise<readonly ArticleReviewArticle[]> {
  const client = createDatabaseClient();

  try {
    return await listReviewableArticles(client.db, { limit: reviewListLimit });
  } finally {
    await client.close();
  }
}

export async function getArticleReview(articleId: string): Promise<LoadArticleReviewResult> {
  const client = createDatabaseClient();

  try {
    return await loadArticleReview(client.db, articleId);
  } finally {
    await client.close();
  }
}

export async function transitionEditorialArticleReviewStatus(input: {
  ...
  try {
    return await transitionArticleReviewStatus(client.db, input);
  } finally {
    await client.close();
  }
}

export async function publishEditorialArticle(articleId: string): Promise<PublishArticleResult> {
  const client = createDatabaseClient();
...
export type { ArticleReviewArticle };
